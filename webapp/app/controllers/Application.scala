...
object Application extends Controller {
  ...
  def searchResult(query: Query) = {
    ...
    Ok(views.html.search(searchForm.fill(query), 
        advancedSearchForm.fill(AdvancedQuery(LuceneQueryExecutor.fullLuceneQueryString(query), query.groupBy)), 
        false, Some(ResultSet(groups)), Some(AdvancedQuery(queryString, query.groupBy))))
  }
  ...
  def evaluate(groupByString: String, query: String) = Action {
    ...
    Ok(groups.zipWithIndex.map { case (group, i) =>
      ...
      Iterable(i, head.arg1, head.rel, head.arg2, groupBy, query).mkString("\t")
    }.mkString("\n"))
  }
  ...
  def sentences(arg1: String, rel:String, arg2:String, corpus: Option[String]) = Action {
    val extrs = LuceneQueryExecutor.executeExact(arg1, rel, arg2, corpus)
